...
class Category:
    ...
    def close_under_composition(self):
        """
        Compute the free category: add all compositions of existing morphisms.
        This is the 'transitive closure' under composition.
        After this, hom(A,B) contains ALL paths from A to B.
        """
        changed = True
        while changed:
            changed = False
            items = list(self._hom.items())
            for (A, B), f_list in items:
                for (B2, C), g_list in items:
                    if B != B2: continue
                    for f in f_list:
                        for g in g_list:
                            comp = self.compose(f, g)
                            key  = (A, C)
                            existing = [m["path"] for m in self._hom.get(key,[])]
                            if comp["path"] not in existing:
                                self._hom.setdefault(key, []).append(comp)
                                changed = True
