...
def compute_pareto_front(G, source, target, weight_fns, max_paths=500):
    """
    ...
    for trial in range(min(max_paths, 200)):
...
def pareto_front_size_experiment(G, source, n_runs=10):
    ...
    2 criteria: small front
    3 criteria: larger front
    4 criteria: potentially exponential
    Returns timing and size data.
    """
...
def verify_semiring_on_pareto(G, source, target):
    """
    Show that the 4 semiring-optimal paths are all Pareto-non-dominated
    — they are ON the Pareto front, just specific points of it.

    This validates the CT/semiring approach: it doesn't miss good paths,
    it selects the optimal one per enrichment criterion.
    """
    weight_fns = [w_distance, w_time, w_fare, w_safety]
    fn_names   = ["distance", "time", "fare", "safety"]
